...
void rwrite(int fd, const void *data, size_t n, struct sockaddr *addr,
	    socklen_t len)
{
...
int main(int argc, char **argv)
{
	...
	int yes = 1;
	assert(!setsockopt(sockfd, SOL_IP, IP_RECVERR, &yes, sizeof(yes)));
	assert(!setsockopt(sockfd, SOL_IP, IP_RECVTTL, &yes, sizeof(yes)));
	assert(!setsockopt(sockfd, SOL_IP, IP_RETOPTS, &yes, sizeof(yes)));
